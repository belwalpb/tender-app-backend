package com.tender.constants;

public class TenderConstants {

    public static final String EMPTY_USER_EXCEPTION_MSG = "User should not be empty";
    public static final String NO_USER_EXISTS_MSG = "No User Exists. Please Register Yourself.";

    public static final String JWT_ISSUER = "TENDER_APP_BACKEND";
    public static final String JWT_TOKEN_KEY = "ABCDs888vvjoijiojoijvijv28989@893ufuid";
    public static final long JWT_TOKEN_VALIDITY =  5 * 60 * 60; //5h * 60 min* 60 min
}
